...
from ragas.metrics import LLMContextRecall, Faithfulness, ContextUtilization, AnswerRelevancy
...
result = evaluate(dataset=evaluation_dataset,metrics=[Faithfulness(), AnswerRelevancy(), ContextUtilization()],llm=evaluator_llm)
print(f"Evaluation Results: {result}")

row_scores_path = f"evaluation_results/ragas_row_scores_{os.getenv('CONFIG_VERSION')}.jsonl"
